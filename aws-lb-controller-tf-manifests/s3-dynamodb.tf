...
resource "aws_s3_object" "dev-bucket-sub1" { # dev bucket
  bucket  = aws_s3_bucket.dev-bucket.id
  key     = "dev/"
  content = ""
}

resource "aws_s3_object" "dev-bucket-sub2" { # eks bucket
  bucket  = aws_s3_bucket.dev-bucket.id
  key     = "/dev/eks-cluster/"
  content = ""
}

resource "aws_s3_object" "dev-bucket-sub3" { # lb controller bucket
  bucket = aws_s3_bucket.dev-bucket.id
  key    = "/dev/aws-lbc/"
  content = ""
}
